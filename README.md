...
- `derived/daily/*.csv`
  - 일봉 + 이동평균(`ma5`, `ma10`, `ma20`, `ma120`, `ma240`)
- `derived/weekly/*.csv`
  - 주봉 집계 + 이동평균 + `is_final`
- `derived/monthly/*.csv`
  - 월봉 집계 + 이동평균 + `is_final`
...
1. `config.py`에서 경로(`DATA_DIR`, `OUTPUT_DIR`) 확인
2. 데이터 업데이트 실행
...
- `scripts/refresh_and_compare.py`, `scripts/scan_and_compare_only.py`는 검증/비교용 보조 스크립트입니다.
